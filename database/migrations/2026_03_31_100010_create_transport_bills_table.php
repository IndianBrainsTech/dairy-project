 ...
 *
 * CONFIRMED FROM DB:
 *   SHOW CREATE TABLE transport_bills
 *   → #1146 Table 'aasaii_db.transport_bills' doesn't exist
 *
 * DEPENDENCY:
 *   supplier_transporters → created in migration 100003
 *   This migration runs AFTER 100003 so dependency is satisfied.
 *
 * THIS MIGRATION CREATES:
 *   1. transport_bills       — bill header
 *   2. transport_bill_items  — bill line rows (one per trip sheet)
 *
 * ALL FK NAMES are short custom names (max 17 chars).
 * MySQL 64-char limit is not a concern.
 * ============================================================
 */
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (!Schema::hasTable('transport_bills')) {
        ...
        if (!Schema::hasTable('transport_bill_items')) {

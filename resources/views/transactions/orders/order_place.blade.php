        ...
        <div class="row">
            <div class="col-sm-12">
                @component('app-components.breadcrumb-3')
                    @slot('title') Place Order @endslot
                    ...
                    <div class="card-body pb-0" style="min-height: 470px; padding-left:30px">
                        ...
                        @csrf
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="row mb-2">
                                        <input type="date" name="invoice_date" id="invoiceDate" value="{{$invoiceDate}}" class="my-control" disabled>
                                        ...
                                        <div class="input-group mr-3" style="width:375px">
                                            ...
                                            <input type="text" id="txtCustomer" class="form-control" placeholder="Customer">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <input type="text" id="txtStock" class="form-control mr-3" style="width:100px;text-align:center" placeholder="Stock" readonly>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        @can('latest_order')
                                        <button type="button" id="btnLastOrder" class="btn btn-gradient-pink waves-effect waves-light btn-sm ml-2" data-toggle="tooltip" data-placement="top" title="Alt+Y">
                                            <i class="mdi mdi-star mr-2"></i>Yesterday Order
                                        </button>
                                        @endcan
                                        <div class="ml-auto">
                                            <button type="button" id="btnSubmit" class="btn btn-primary btn-sm px-3 mr-4" data-toggle="tooltip" data-placement="top" title="Alt+O">
                                                <i class="mdi mdi-shopping mr-2"></i>Place Order
                                            </button>
                                        </div>
                                    </div>
                                </div>
        ...
        $(document).ready(function () {
            ...
            $('#route').change(function () { 
                ...
                $.get(url, function (data) {
                    ...
                    $("#txtCustomer").autocomplete('option', 'source', autocompleteSource(customers));
                });
            });
            ...
            $("#txtCustomer").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    getOutStandingAmount(id);
                }
            });

            function getOutStandingAmount(id) {
                console.log("Id : " + id); 
            ...
            $("#txtProduct").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    let id = products.get(key);
                    let price = getPriceString(id);
            ...
            $("#btnAdd").click(function(e){
                ...
                else if(convertQty > cStock.current_stock ) Swal.fire('Attention', 'This product is out of stock.', 'error');
                else {
                    var productId = products.get(product);
                    if(productId) {
            ...
            @can('latest_order')
            $("#btnLastOrder").click(function(e){
                var customerId = $("#customer").val();
                if(customerId) {
                    clearOrderInfo();
                    let url = "{{ route('orders.last', ':cust_id') }}".replace(':cust_id', customerId);
                    $.get(url, function (data) {
                        loadOrderItems(data.orderItems, true);                        
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data.success){ 
                                Swal.fire({
                                        title: 'Success!',
                                        html: "Order Placed Successfully!<br/>Order Number : " + data.order_num,
                                        icon: 'success'
                                    }
                                )
                                .then(
            ...
            $(document).on('keydown', function(event) {
                if (event.altKey && event.key.toUpperCase() === 'O')
                    $('#btnSubmit').click();
                else if (event.altKey && event.key.toUpperCase() === 'Y')
                    $('#btnLastOrder').click();
            });
            ...
            function updateSalesInvoiceTable(invAmt, discount) {
                ...
                invAmt = parseFloat(invAmt.toFixed(2));
                tcsAmt = parseFloat(tcsAmt.toFixed(2));
                invAmt += tcsAmt;
            ...
            function loadOrderItems(orderItems, checkStock=false) {
                ...
                for (var i = 0; i < orderItems.length; i++) {
                    ...
                    if(checkStock) {
                        var convertQty = convertToPrimary(productId, qty, unitId);
                        var cStock = getCurrentStock(productId, category);                        
                        if(convertQty > cStock.current_stock)
                ...
                if (outOfStockItems.length > 0) {
                    Swal.fire('Sorry!', `${outOfStockItems.join(", ")} are out of stock.`, 'warning');
                }
            ...
            {
                var primQty = 0;
                var productUnit = productUnits.filter(function(item) { return item.product_id == productId; });
            ...
            function getCurrentStock(productId, category = "") {                
                let stockItem = getStockByProductId(productId);                
                if(category=="") category = $('#selectCategory').val();
                let stockQty = 0;

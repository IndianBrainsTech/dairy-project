...
@section('title', 'View Bulk Milk Order')
        ...
        <div class="row">
            <div class="col-sm-12">
                @component('app-components.breadcrumb-3')
                    @slot('title') View Bulk Milk Order @endslot
            ...
            <div class="col-lg-10 mx-auto">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="px-2">
                            <div class="row my-2">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mt-2">{{ getIndiaDateTime($order['order_dt']) }}</div>
                                </div>
                                <div class="col-md-3 text-right">
                                    @if($order['order_status'] != "Cancelled" && $order['invoice_status'] == "Not Generated" && intval($order['net_amt']) == 0)
                            ...
                            <div class="row my-2">
                                ...
                                <div class="col-md-9">{{ getIndiaDate($order['invoice_date']) }}</div>
                            </div>
                            <div class="row my-2">
                                <div class="col-md-3">
                                    {{ $order['order_status'] == "Cancelled" ? 'Order Status' : 'Invoice Status' }}
                                </div>
                                <div class="col-md-9">
                                    {{ getOrderStatus($order['order_status'] == "Cancelled" ? $order['order_status'] : $order['invoice_status']) }}
                            ...
                            <div class="row my-2">
                                ...
                                <div class="col-md-9">{{ $order['route'] }}</div>
                            </div>
                            <div class="row my-2">
                                <div class="col-md-3">Vehicle Number</div>
                                <div class="col-md-9">{{ $order['vehicle_num'] }}</div>
                            </div>
                            <div class="row my-2">
                                <div class="col-md-3">Driver Name</div>
                                <div class="col-md-9">{{ $order['driver_name'] }}</div>
                            </div>
                            <div class="row my-2">
                                <div class="col-md-3">Mobile Number</div>
                                <div class="col-md-9">{{ $order['driver_mobile_num'] }}</div>
                            </div>
                            <div class="row my-2">
                        ...
                        <!-- Order Table -->
                        <h6 class="my-heading pl-2 pt-3">Order Data :</h6>
                                ...
                                <tbody>
                                    @foreach($order_items as $orderItem)
                                        <tr style='height:32px'>
                                            ...
                                            <td class="pl-2">{{ $orderItem->product_name }}</td>
                                            <td class="text-center">{{ $orderItem->hsn_code }}</td>
                                            <td class="text-center">{{ $orderItem->qty_kg }}</td>
                                            <td class="text-center">{{ $orderItem->clr }}</td>
                                            <td class="text-center">{{ $orderItem->fat }}</td>
                                            <td class="text-right">{{ $orderItem->snf }}</td>
                                            <td class="text-right">{{ $orderItem->qty_ltr }}</td>
                                            <td class="text-right">{{ $orderItem->ts }}</td>
                                            <td class="text-center">{{ $orderItem->ts_rate }}</td>
                                            <td class="text-right">{{ $orderItem->rate }}</td>
                                            <td class="text-right pr-2">{{ getTwoDigitPrecision($orderItem->amount) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr id="totalRow">
                                        ...
                                        <th id="totQtyKg" class="text-center">{{ $order_items->sum('qty_kg') }}</th>
                                        <td colspan="3"></td>
                                        <th id="totQtyLtr" class="text-right">{{ $order_items->sum('qty_ltr') }}</th>
                                        <td colspan="3"></td>
                                        <th id="totAmt" class="text-right pr-2">{{ getTwoDigitPrecision($order_items->sum('amount')) }}</th>
                                    </tr>
                                    <tr>
                                        <td colspan="11" class="text-right pr-2 border-bottom-0">TCS</td>
                                        <td id="tcs" class="text-right pr-2">{{ getTwoDigitPrecision($order['tcs']) }}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="11" class="text-right pr-2 border-top-0 border-bottom-0">Round Off</td>
                                        <td id="roundOff" class="text-right pr-2">{{ getTwoDigitPrecision($order['round_off']) }}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="11" class="text-right pr-2 border-top-0">Net Amount</td>
                                        <th id="netAmt" class="text-right pr-2">{{ getTwoDigitPrecision($order['net_amt']) }}</th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                        ...
                        @if($order['order_status'] != "Cancelled" && $order['invoice_status'] == "Not Generated")
        ...
        $(document).ready(function () {
            ...
            $('#btnOrderCancel').on("click", function () {
                ...
                if(!remarks) {
                    ...
                    $.ajax({
                        url: "{{ route('bulk-milk.orders.cancel') }}",
                        type: "GET",
                        data: { 
                        ...
                        success: function (data) {
                            Swal.fire('Success!', data.message, 'success')
                                .then(() => window.location.replace("{{ route('bulk-milk.orders.index') }}"));
                        },
                        error: function (data) {
            ...
            $('#btnEdit').on("click", function () {
                ...
                let form = $('<form>', {
                    'method': 'POST',
                    'action': "{{ route('bulk-milk.orders.edit') }}"
                });

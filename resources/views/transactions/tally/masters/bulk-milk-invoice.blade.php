        ...
        <IMPORTDATA>
            ...
            <REQUESTDATA>
                <TALLYMESSAGE xmlns:UDF="TallyUDF">
                    <VOUCHER REMOTEID="{{ $invoice->remote_id }}" VCHTYPE="Bulk Milk Sales" ACTION="Create">
                        <DATE>{{ str_replace('-', '', $invoice->invoice_date) }}</DATE>
                        <PARTYNAME>{{ $invoice->customer->customer_name }}</PARTYNAME>
                        <VOUCHERTYPENAME>Bulk Milk Sales</VOUCHERTYPENAME>
                        ...
                        @foreach($invoiceItems as $item)
                            <ALLINVENTORYENTRIES.LIST>
                                ...
                                <ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>
                                <RATE>{{ $item->amount / $item->qty_ltr }}/{{ $item->unit }}</RATE>
                                <AMOUNT>{{ $item->amount }}</AMOUNT>
                                <ACTUALQTY>{{ $item->qty_ltr }} {{ $item->unit }}</ACTUALQTY>
                                <BILLEDQTY>{{ $item->qty_ltr }} {{ $item->unit }}</BILLEDQTY>
                                <ACCOUNTINGALLOCATIONS.LIST>
                                    <LEDGERNAME>Bulk Milk Sales</LEDGERNAME>
                                    <HSNCODE>{{ $item->hsn_code }}</HSNCODE>
                                    <ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>
                                    <AMOUNT>{{ $item->amount }}</AMOUNT>
                                </ACCOUNTINGALLOCATIONS.LIST>

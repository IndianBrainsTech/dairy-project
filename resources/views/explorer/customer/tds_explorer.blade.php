                    ...
                    <div class="card-body">
                        <div style="width:100%">
                            <div style="width:60%;float:left">
                                <h4 class="header-title mt-0">TDS Status &nbsp;
                        ...
                        <form action="{{ route('tds.explorer') }}" method="POST" class="float-right">
                            @csrf
                            <div class="d-flex align-items-center">
                                <label class="mr-2 text-nowrap">TDS Status</label>
                                <select name="tds_status" class="form-control" style="min-width: 100px">
                                    <option value="All" @selected(old('tds_status', $tds_status) == 'All')>All</option>
                            ...
                            <table id="datatable" class="table table-sm table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th data-priority="2" class="text-center">TDS Status</th>
                                        <th data-priority="3" class="text-center">PAN Number</th>
                                        <th data-priority="4" class="text-center" style="min-width: 80px">TDS</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($customers as $customer)
                                        <tr>
                                            ...
                                            <td >{{ $customer->tds_status == "TDS Applied" ? "Already in TDS" : $customer->tds_status }}</td>
                                            <td>{{ $customer->pan_number }}</td>
                                            <td class="text-center">
                                                @if($customer->tds_status != "TDS Not Applicable")
                                                    {{ $customer->pan_number ? $tdsMaster->with_pan."%" : $tdsMaster->without_pan."%" }}
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach

...
@section('title', 'Secondary Transport')
@section('headerStyle')
...
<div class="container-fluid">
    <div class="row"><div class="col-12">
        @component('app-components.breadcrumb-4')
            @slot('title') Secondary Transport @endslot
            @slot('item1') Transport @endslot @slot('item2') Transactions @endslot @slot('item3') Secondary Transport @endslot
        @endcomponent
    </div></div>
    ...
    <div class="card"><div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('transport.secondary-transport.create') }}" class="btn btn-primary px-3">
        ...
        <div class="table-responsive">
            <table id="datatable" class="table table-bordered table-sm table-hover dt-responsive nowrap w-100">
                <thead class="thead-light"><tr>
                    ...
                    <th>Reference</th><th>Date</th><th>Transporter</th><th>Vehicle No.</th><th class="text-right">Total Amt</th><th class="text-center">Status</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $record->transport_date->format('d-m-Y') }}</td>
                        <td>{{ $record->supplierTransporter->name ?? '—' }}</td>
                        <td>{{ $record->vehicle_number }}</td>
                        <td class="text-right">₹ {{ number_format($record->total_amount,2) }}</td>
                        <td class="text-center">
                            @php $sc = ['pending'=>'warning','billed'=>'success','cancelled'=>'danger'][$record->status] ?? 'secondary'; @endphp
                            <span class="badge badge-soft-{{ $sc }}">{{ ucfirst($record->status) }}</span>
                        </td>
                        <td class="text-center">
                            <a href="{{ route('transport.secondary-transport.show', $record) }}" class="mr-1"><i class="dripicons-preview text-primary font-18"></i></a>
                            <a href="{{ route('transport.secondary-transport.edit', $record) }}"><i class="dripicons-pencil text-warning font-18"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>

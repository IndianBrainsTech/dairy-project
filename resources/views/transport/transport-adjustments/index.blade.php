...
@section('title', 'Transport Adjustments')
@section('headerStyle')
...
<div class="container-fluid">
    <div class="row"><div class="col-12">
        @component('app-components.breadcrumb-4')
            @slot('title') Transport Adjustments @endslot
            @slot('item1') Transport @endslot @slot('item2') Transactions @endslot @slot('item3') Adjustments @endslot
        @endcomponent
    </div></div>
    ...
    <div class="card"><div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('transport.transport-adjustments.create') }}" class="btn btn-primary px-3">
        ...
        <div class="table-responsive">
            <table id="datatable" class="table table-bordered table-sm table-hover dt-responsive nowrap w-100">
                <thead class="thead-light"><tr>
                    ...
                    <th>Adj. No.</th><th>Date</th><th>Vehicle</th><th>Type</th><th class="text-right">Amount</th><th class="text-center">Status</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $record->adjustment_number }}</td>
                        <td>{{ $record->adjustment_date->format('d-m-Y') }}</td>
                        <td>{{ $record->vehicle->vehicle_number ?? '—' }}</td>
                        <td>
                            @if($record->adjustment_type==='debit')<span class="badge badge-soft-danger">Debit</span>
                            @else<span class="badge badge-soft-success">Credit</span>@endif
                        </td>
                        <td class="text-right">₹ {{ number_format($record->amount,2) }}</td>
                        <td class="text-center">
                            @php $sc = ['pending'=>'warning','approved'=>'success','rejected'=>'danger'][$record->status] ?? 'secondary'; @endphp
                            <span class="badge badge-soft-{{ $sc }}">{{ ucfirst($record->status) }}</span>
                        </td>
                        <td class="text-center">
                            <a href="{{ route('transport.transport-adjustments.show', $record) }}" class="mr-1"><i class="dripicons-preview text-primary font-18"></i></a>
                            <a href="{{ route('transport.transport-adjustments.edit', $record) }}"><i class="dripicons-pencil text-warning font-18"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>

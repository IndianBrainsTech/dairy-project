...
@section('title', 'Secondary Transport Bills')
@section('headerStyle')
...
<div class="container-fluid">
    <div class="row"><div class="col-12">
        @component('app-components.breadcrumb-4')
            @slot('title') Secondary Transport Bills @endslot
            @slot('item1') Transport @endslot @slot('item2') Billing @endslot @slot('item3') Secondary Bills @endslot
        @endcomponent
    </div></div>
    ...
    <div class="card"><div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('transport.secondary-transport-bills.create') }}" class="btn btn-primary px-3">
        ...
        <div class="table-responsive">
            <table id="datatable" class="table table-bordered table-sm table-hover dt-responsive nowrap w-100">
                <thead class="thead-light"><tr>
                    ...
                    <th>Bill No.</th><th>Date</th><th>Transporter</th><th class="text-right">Net Payable</th><th class="text-right">Balance</th><th class="text-center">Status</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right text-danger">₹ {{ number_format($record->balance_amount,2) }}</td>
                        <td class="text-center">
                            @php $sc = ['draft'=>'warning','approved'=>'success','cancelled'=>'danger'][$record->status] ?? 'secondary'; @endphp
                            <span class="badge badge-soft-{{ $sc }}">{{ ucfirst($record->status) }}</span>
                        </td>
                        <td class="text-center">
                            <a href="{{ route('transport.secondary-transport-bills.show', $record) }}" class="mr-1"><i class="dripicons-preview text-primary font-18"></i></a>
                            <a href="{{ route('transport.secondary-transport-bills.edit', $record) }}"><i class="dripicons-pencil text-warning font-18"></i></a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>

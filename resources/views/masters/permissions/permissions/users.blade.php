        ...
        <div class="row">
            <div class="col-sm-12">
                @component('app-components.breadcrumb-3')
                    @slot('title') User Permissions @endslot
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row px-3">
                            <label for="ddl-user" class="col-form-label mr-3">User <small class="text-danger font-13">*</small></label>
                            <select id="ddl-user" class="app-control mr-2" style="min-width: 200px; padding: 6px 10px" required>
...
@push('custom-scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            function doInit() {
                $("#div-message").text("No User Selected!");
                $('#div-granted').hide();
            ...
            function getAndLoadPermissions() {
                userId = $('#ddl-user').val();
                if(!userId) {
                    Swal.fire('Sorry!', "Please select user" ,'warning');
                }
                else {
                    $('input[name="permissions[]"]').prop('checked', false);

                    $.ajax({
                        url: "{{ route('permissions.user-permissions.show', ['user' => '__ID__']) }}".replace('__ID__', userId),
                        method: "GET",
                        dataType: "json"
                    })
                    .done(response => {
            ...
            function validateAndSavePermissions() {
                ...
                if(!userId) {
                    ...
                    if (!selectedPermissions || selectedPermissions.length === 0) {
                        const user = $('#ddl-user option:selected').text();
                        Swal.fire({
                            title: 'Confirm?',
                            html: `No Permissions Selected!<br/>Do you want to revoke all permissions for ${user}?`,
                            icon: 'question',
                        ...
                        .then((result) => {
                            if (result.value)
                                savePermissions(userId, selectedPermissions);
                        });
                    }
                    else {
                        savePermissions(userId, selectedPermissions);
            ...
            function showGrantedPermissions() {
                ...
                if (!permissions?.length) {
                    $("#div-message")
                        .text(`${user} does not have any permissions yet.`)
                ...
                $("#div-message")
                    .text(hasAll ? `${user} has all permissions.` : `Permissions of ${user}`)
            ...
            function savePermissions(userId, selectedPermissions) {
                $('#btn-save').prop('disabled', true);
                $.ajax({
                    url: "{{ route('permissions.user-permissions.update', ['user' => '__ID__']) }}".replace('__ID__', userId),
                    method: "PUT",

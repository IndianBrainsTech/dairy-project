...
@section('content')
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-body">
                        <div style="width:100%">
                            <div style="width:60%;float:left"><h4 class="header-title mt-0">TCS Master</h4></div>
                            <div style="width:40%;float:left"><button type="button" id="add_tcs" class="btn btn-gradient-primary px-4 float-right mt-0 mb-3" data-toggle="modal" data-animation="bounce" data-target="#modal_tcs"><i class="mdi mdi-plus-circle-outline mr-2"></i>New TCS Data</button></div>
                        </div>
                        <div class="table-responsive dash-social">
                            <table id="datatable" class="table table-bordered text-center">
                                ...
                                <tbody>
                                    @foreach($tcs_masters as $tcs_data)
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td>{{ displayDate($tcs_data->effect_date) }}</td>
                                            <td>{{ formatNumberWithCommas($tcs_data->tcs_limit) }}</td>
                                            <td>{{ $tcs_data->with_pan }} %</td>
                                            <td>{{ $tcs_data->without_pan }} %</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>                    
    ...
    <!-- Start of Tax Modal -->
    <div class="modal fade" id="modal_tcs" tabindex="-1" role="dialog" aria-labelledby="modalTaxLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="modal_tcs_title">Add TCS Data</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="form_tcs">
                    <input type="hidden" id="tcs_id" name="tcs_id" value="">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="form-group row">
                                    <label for="tcs_limit" class="col-sm-5 col-form-label">TCS Limit <small class="text-danger font-13">*</small></label>
                                    <div class="col-sm-5">
                                        <input type="number" class="form-control" id="tcs_limit" name="tcs_limit" required="" maxlength="10">                                        
                                    </div>
                                </div>                                
                                <div class="form-group row">
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="submit" class="btn btn-primary" id="submit" value="Add TCS Data"/>
                    </div>
                </form>
            </div>
        </div>
...
@push('custom-scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('body').on('click', '#add_tcs', function (event) {
                ...
                $('#modal_tcs').modal('show');
            });

            function isValid(effect_date,tcs_limit,with_pan,without_pan) {
                let valid = false;                
                if(!effect_date) {
                    Swal.fire('Attention','Please Choose Effect Date','error');
                }
                else if(!tcs_limit) {
                    Swal.fire('Attention','Please Enter TCS Limit','error');
                }
                else if(!with_pan) {
            ...
            $('body').on('click', '#submit', function (event) {
                ...
                var tcs_limit = $("#tcs_limit").val();
                var with_pan = $("#with_pan").val();
                var without_pan = $("#without_pan").val();
                
                tcs_limit = parseFloat(tcs_limit);
                with_pan = parseFloat(with_pan);
                without_pan = parseFloat(without_pan);

                if(isValid(effect_date,tcs_limit,with_pan,without_pan)) {
                    $.ajax({
                        url: "{{ route('tcsmaster.store') }}",
                        type: "POST",
                        data: {
                            id: "0",
                        ...
                        success: function (data) {               
                            $('#form_tcs').trigger("reset");
                            $('#modal_tcs').modal('hide');                                                
                            Swal.fire({
                                    title:'Success!',
                                    text:"TCS Data has been generated!",
                                    type:'success'
                                }
                            )
                            .then(

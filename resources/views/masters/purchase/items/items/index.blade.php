...
@section('title', 'Purchase Items Master')
        ...
        <div class="row">
            <div class="col-12">
                @component('app-components.breadcrumb-4')
                    @slot('title') Item Masters @endslot
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3">
                            ...
                            <button type="button" class="btn btn-pink btn-round font-weight-medium px-3 mb-2 mb-md-0">
                                {{ $masters->count() }} {{ Str::plural('Item', $masters->count()) }}
                            </button>
                            ...
                            <div class="d-flex flex-column flex-sm-row gap-2">
                                ...
                                <!-- Create button -->
                                <a href="{{ route('purchase.items.items.create') }}" class="btn btn-primary px-3 mb-2 mb-sm-0">
                            ...
                            <table id="datatable" class="table table-bordered table-sm table-hover dt-responsive nowrap w-100">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="text-left pl-2">Group</th>
                                ...
                                <tbody>
                                    @foreach($masters as $master)
                                        <tr>
                                            ...
                                            <td class="text-left pl-2">{{ $master->group->name }}</td>
                                            <td class="text-center">{{ $master->hsn_code }}</td>
                                            @if($status != "Active")
                                                <th class="text-center">{!! getStatusWithBadge($master->status->label()) !!}</th>
                                            @endif
                                            <td class="text-center">
                                                <a href="{{ route('purchase.items.items.show', ['master' => $master->id]) }}">
...
@push('custom-scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            setMenuItemActive('Masters','ul-purchase-items','li-purchase-item');
            ...
            $('.dropdown-menu .dropdown-item').on('click', function (e) {
                ...
                if(newStatus != oldStatus) {
                    window.location.href = "{{ route('purchase.items.items.list') }}" + "?status=" + encodeURIComponent(newStatus);
                }
            });
        });
    </script>

...
@section('headerStyle')
    ...
    <style type="text/css">
        ...
        .table-container input.price-cell {
            color: black;
            max-width: 80px;
            font-weight: 500;
            padding-top: 1px;
...
@php
    if(!isset($price_list)) {
        $isEdit = false;
        $title = "Add Price Master";
        $action = route('price-master.store');
        $effect_date = "";
        $narration = "";
        ...
        $isEdit = true;
        $title = "Edit Price Master";
        $action = route('price-master.update', ['id' => $id]);
    }
@endphp
                                        ...
                                        <tbody>
                                            @foreach($products as $product)
                                                <tr>
                                                    ...
                                                    <td>{{ $product->name }}</td>
                                                    <td><input type="text" name="prod[{{$product->id}}]" id="prod{{$product->id}}" class="form-control price-cell" data-id="{{$product->id}}" tabindex="{{$loop->index + 1}}" maxlength="8"></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
    ...
    <script>
        $(document).ready(function () {
            ...
            if(@json($isEdit)) {
                ...
                // Update Price List
                var priceList = @json($price_list);
                for (var productId in priceList) {
                    if (priceList.hasOwnProperty(productId)) {
                        var price = priceList[productId];
                        $('input[id="prod' + productId + '"]').val(price);                        
                    }
                }
            ...
            $(".price-cell").keypress(function(e) {
            ...
            function collapsePriceList() {
                $(".price-cell").each(function (index, element) {
                    var value = $(this).val();
                    if(!value) {
            ...
            function getPriceList() {
                ...
                $('[id^=prod]').each(function() {
                    ...
                    if (value.trim() !== '') { // Check if the value is not empty
                        priceData[id] = value; // Add to the collection
                    }                    
                });
                return priceData;
            }
            ...
            $('#submit').on('click', function (event) {
                ...
                if(!effect_date) {
                ...
                else if(Object.keys(priceList).length == 0) {
                    Swal.fire('Attention','Please Update Price List','warning');
                }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            .then(
                                function() { 
                                    window.location.replace("{{ route('price-master.index') }}");
                                }
                            );  
                        },

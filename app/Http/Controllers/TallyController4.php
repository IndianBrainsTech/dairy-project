...
/* -------------------------------------------------------------- */
    ...
    private function tallySalesInvoice($invoiceNum)
    {
        ...
        $xmlDeclaration = '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL;
        $xmlView = view('transactions.tally.masters.exempted-invoice', compact('invoice','invoiceItems'));
    ...
    private function tallyTaxInvoice($invoiceNum)
    {        
        ...
        $xmlDeclaration = '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL;
        $xmlView = view('transactions.tally.masters.tax-invoice', compact('invoice','invoiceItems'));
        $xmlContent = $xmlDeclaration . trim($xmlView->render());
    ...
    {
        $invoice = BulkMilkOrder::select('id','invoice_date','invoice_num','customer_id','tot_amt','tcs','round_off','net_amt','created_at','order_dt')
                ->with('customer:id,customer_name,customer_code')
                ->where('invoice_num',$invoiceNum)
                ->first();
        $invoice->remote_id = $invoice->invoice_num . "-" . preg_replace('/\D/', '', $invoice->order_dt);

        $invoiceItems = BulkMilkOrderItem::select('id','product_id','product_name','hsn_code','qty_ltr','amount')
            ->where('invoice_num',$invoiceNum)
            ->get();
        ...
        $xmlDeclaration = '<?xml version="1.0" encoding="UTF-8"?>' . PHP_EOL;
        $xmlView = view('transactions.tally.masters.bulk-milk-invoice', compact('invoice','invoiceItems'));
        $xmlContent = $xmlDeclaration . trim($xmlView->render());

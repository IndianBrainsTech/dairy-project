...
class ItemwiseSalesExport implements FromArray, WithTitle, WithHeadings, WithProperties, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    protected $heading;
    protected $titles;
    protected $data;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $sheet->getStyle("A{$lr}:B{$lr}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                ...
                $lastColumn = "{$lc}4:{$lc}".($lr-1);
                $sheet->getStyle($lastColumn)->getAlignment()->setIndent(0)->setHorizontal(Alignment::HORIZONTAL_CENTER);

                // Format Numbers
                $c1 = Coordinate::stringFromColumnIndex($cc-2); // Total Qty Column
                $c2 = Coordinate::stringFromColumnIndex($cc-1); // Total Amount Column
                $numCells = "C4:{$c1}".($lr);

...
class TaxwiseSummaryExport implements FromArray, WithTitle, WithHeadings, WithProperties, WithEvents, WithCustomStartCell, ShouldAutoSize
{
    protected $heading;
    protected $titles;
    protected $type;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                // Freeze Pane
                $fr = ($this->type == "Format1") ? "E4" : "C4"; 
                $sheet->freezePane($fr);

                // Total Row                
                $totalRow = "A{$lr}:{$lc}{$lr}";
                $cl = ($this->type == "Format1") ? "D" : "B";
                ...
                $cl = ($this->type == "Format1") ? "C" : "B";
                $hsnColumn = "{$cl}4:{$cl}{$lr1}";
                $sheet->getStyle($hsnColumn)->getAlignment()->setIndent(0)->setHorizontal(Alignment::HORIZONTAL_CENTER);
                if($this->type == "Format1")
                    $sheet->getStyle("E4:H{$lr1}")->getAlignment()->setIndent(0)->setHorizontal(Alignment::HORIZONTAL_CENTER);
